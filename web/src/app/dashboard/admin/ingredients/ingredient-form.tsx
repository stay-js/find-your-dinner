...
import type { CreateUpdateIngredientSchema, Ingredient } from '~/lib/zod/schemas';
...
type IngredientFormProps = {
  ...
  isOpen: boolean;
  isPending: boolean;
  onSubmit: (values: CreateUpdateIngredientSchema) => void;
  selected: Ingredient | null;
  setIsOpen: (open: boolean) => void;
  setSelected: React.Dispatch<React.SetStateAction<Ingredient | null>>;
};
...
export function IngredientForm({
  ...
  return (
    <Sheet onOpenChange={setIsOpen} open={isOpen}>
      ...
      >
        <SheetHeader>
          <SheetTitle>Hozzávaló {isEditing ? 'szerkesztése' : 'létrehozása'}</SheetTitle>
          <SheetDescription>
            A következő űrlap segítségével{' '}
            {isEditing ? 'szerkesztheted a kiválaszott ' : 'hozhatsz létre új '}
            hozzávalót. A mentés gombra kattintva tudod rögzíteni a változtatásokat.
          </SheetDescription>
        </SheetHeader>
